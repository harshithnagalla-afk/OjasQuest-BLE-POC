  ...
  return (
    <div className="app-shell">
      ...
      <header className="topbar">
        ...
        <div>
          <div className="identity-name">{user.displayName ?? user.email}</div>
          <div className="identity-email">{user.email}</div>
        </div>
        <button className="btn btn-ghost spacer" onClick={() => signOutUser()}>
          Sign out
        </button>
      </header>

      <section className="card">
        <h2 className="card-title">Bluetooth device</h2>

        <div className="scan-control">
          <div className={`scan-orb${isScanning ? ' is-scanning' : ''}`}>
            <span className="scan-ring" />
            <span className="scan-ring delay" />
            <button
              className={`scan-orb-core${isConnected ? ' is-connected' : ''}`}
              onClick={isConnected ? handleDisconnectClick : handleConnectClick}
              disabled={isScanning}
            >
              {isScanning ? 'Scanning…' : isConnected ? 'Connected' : 'Connect'}
            </button>
          </div>
          ...
          {isConnected ? (
            <>
              ...
              <p className={`pairing-note${pairResult === 'error' ? ' error' : ''}`}>
                {pairResult === 'pairing' && 'Linking to your account…'}
                {pairResult === 'paired' && 'Linked to your account'}
                {pairResult === 'error' && 'Could not link to your account'}
              </p>
              <button className="btn btn-danger-outline" onClick={handleDisconnectClick}>
                Disconnect
              </button>
            </>
          ) : (
            <div className="status-row">
              ...
              <span className="status-label">
                {status === 'connecting' ? 'Scanning for nearby devices' : status === 'error' ? 'Error' : 'Not connected'}
              </span>
            </div>
          )}

          {statusMessage && !isConnected && (
            <p className={`status-message${status === 'error' ? ' error' : ''}`}>{statusMessage}</p>
          )}
        </div>
      </section>
      ...
      {isConnected && (
        <section className="card">
          <h2 className="card-title">Live panel</h2>
          ...
          <div className="terminal">
            {log.length === 0 ? (
              <div className="terminal-empty">No activity yet.</div>
            ) : (
              log.map((line, i) => (
